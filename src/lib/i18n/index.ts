...
tryDemo|Try the demo|Probar la demo|Demo ausprobieren|Essayer la démo
demoSignInBody|Explore the workspace with one click, or sign in with your credentials.|Explora el espacio de trabajo con un clic o inicia sesión con tus credenciales.|Den Arbeitsbereich mit einem Klick erkunden oder mit Zugangsdaten anmelden.|Explorez l’espace de travail en un clic ou connectez-vous avec vos identifiants.
demoSessionNote|Each demo session has its own saved runs and history. A new session starts fresh.|Cada sesión de demo tiene sus propias ejecuciones e historial. Una sesión nueva empieza desde cero.|Jede Demo-Sitzung hat eigene gespeicherte Läufe und einen eigenen Verlauf. Eine neue Sitzung beginnt von vorn.|Chaque session de démo possède ses propres exécutions et son historique. Une nouvelle session repart de zéro.
orUseCredentials|Or use your credentials|O usa tus credenciales|Oder mit Zugangsdaten anmelden|Ou utilisez vos identifiants
demoAuthError|Demo sign-in failed. Try again shortly or use your credentials.|No se pudo abrir la demo. Reinténtalo en unos instantes o usa tus credenciales.|Demo-Anmeldung fehlgeschlagen. In Kürze erneut versuchen oder Zugangsdaten verwenden.|La connexion à la démo a échoué. Réessayez dans un instant ou utilisez vos identifiants.
